...
(define (synth-solve xs pre post
                     #:guess-first-cex [guess-first-cex? #f])
  ...
  (parameterize ([current-custodian cust])
    ...
    (cond
      [guess-first-cex?
       (solver-assert guesser pre)
       (define x (solver-check guesser))
       (solver-clear guesser)
       (if (sat? x)
           (values (ret (evaluate p (solution/only-inputs x)) #f) ret)
           (values (ret p #t) ret))]
      [else
       (values (ret p #t) ret)])))
