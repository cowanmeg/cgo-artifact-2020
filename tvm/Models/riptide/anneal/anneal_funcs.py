        ...
        if self.quantize:
            if self.fixed:
                ...
                self.alpha = self.add_variable(
                    ...
                    regularizer=tf.keras.regularizers.l2(l))
...
class Scalu(tf.keras.layers.Layer):
    def __init__(self):
...
# Convenience functions for converting to TVM graph
def sawb_quantize(weight_bits, kernel):
    c1, c2 = get_sawb_coefficients(weight_bits)
    alpha = c1 * tf.sqrt(tf.reduce_mean(kernel**2)) + c2 * tf.reduce_mean(tf.abs(kernel))
    fp = SAWBQuantize(kernel, alpha, weight_bits)
    scale = tf.math.reduce_min(tf.abs(fp))
    x = 2 ** weight_bits - 1
    bipolar = fp / scale
    unipolar = (bipolar - x) / 2 + x
    return unipolar, scale
