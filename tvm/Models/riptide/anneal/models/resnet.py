...
class resnet18(tf.keras.Model):
    def __init__(self, classes=1000):
        ...
        self.block1_down_conv = SAWBConv2D(
            filters=64,
            kernel_size=1,
            strides=1,
            padding='valid',
    ...
    def call(self, inputs, training=None):
        # Input layer
        # Note: a2w1 expects this extra name scope..
        if self.config.a_bits == 2 and self.config.w_bits == 1:
        ...
        x = self.flatten(x)
        x = self.dense(x)
        if self.config.a_bits == 1 and self.config.w_bits == 1:
            x = self.scalu(x)
        x = self.softmax(x)
